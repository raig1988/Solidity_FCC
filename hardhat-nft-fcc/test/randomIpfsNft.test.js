...
!developmentChains.includes(network.name) ?
    ...
    describe("Random Ipfs Nft Test", function() {
        let deployer, accounts, randomIpfsNft, vrfCoordinatorV2Mock, mintFee;
        ...
        describe("constructor", function() {
            it("sets starting values correctly", async function() {
                const dogTokensUris = await randomIpfsNft.getDogTokenUris(0);
                assert.equal(dogTokensUris, "ipfs://QmaVkBn2tKmjbhphU7eyztbvSQU5EXDdqRyXZtRhSGgJGo");
        ...
        describe("request Nft", function() {
            it("fails if payment isnt sent with the request", async function() {
                await expect(randomIpfsNft.requestNft()).to.be.revertedWithCustomError(randomIpfsNft, "RandomIpfsNft_NeedMoreEthSent");
            })
            ...
            it("reverts if payment amount is less than the mint fee", async function() {
                await expect(randomIpfsNft.requestNft({value : mintFee.sub(ethers.utils.parseEther("0.001"))}))
                    .to.be.revertedWithCustomError(randomIpfsNft, "RandomIpfsNft_NeedMoreEthSent");
            })
            ...
            it("mints NFT after random number is returned", async function () {
                await new Promise(async (resolve, reject) => {
                    ...
                    try {
                        ...
                        console.log("Running fulfillRandomWords...");
                        await vrfCoordinatorV2Mock.fulfillRandomWords(
                            requestNftReceipt.events[1].args.requestId,
                            randomIpfsNft.address
                        )
                        console.log("Finished fulfillrandomWords")
                        resolve();
                    } catch (e) {
                        console.log(e)
                        reject(e)
        ...
        describe("getBreedFromModdedRng", () => {
            ...
            it("should revert if moddedRng > 99", async function () {
                await expect(randomIpfsNft.getBreedFromModdedRng(100)).to.be.revertedWithCustomError(randomIpfsNft,
